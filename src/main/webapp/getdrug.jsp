...
<div class="row border">
    <div class="col-2 text-center border border-left-0">
        ...
        <div class="mt-4 bg-primary rounded-lg">
            <a href="getdrug" class="text-decoration-none bolder  text-white">药品名单</a>
        </div>
    </div>
    <div class="col border border-left-0 border-top-0">
        <div class="mt-2">
            <form id="excelform" enctype="multipart/form-data" class="mt-2">
                导入药品文件：<input type="file" name="excelfile" id="file_per">
        ...
        <c:choose>
            <c:when test="${!empty drugRepository}">
                <div class=" shadow mt-5">
                    <table class="table text-center text-light">
                        ...
                        <tr>
                            ...
                            <th scope="col">存储数量</th>
                            <th scope="col">选择</th>
                        </tr>
                        </thead>
                        <tbody class="text-dark">
                        <c:forEach items="${drugRepository}" var="item">
                            <tr id="drugId${item.drug_id}">
                                <td><label for="" class="mt-2">${item.drug_id}</label></td>
                                <td><label for="" class="mt-2">${item.drug_name}</label></td>
                                <td><label for="" class="mt-2">${item.drug_price}</label></td>
                                <td><label for="" class="mt-2">${item.drug_num}</label></td>
                                <td><button  onclick="deleteWorker('${item.drug_id}')" type="submit" class="btn btn-danger">删除</button></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
...
<%--                        <td>${item.drug_num}</td>--%>
<%--                        <td>--%>
<%--                            <button onclick="deleteWorker('${item.drug_id}')">删除</button>--%>
<%--                        </td>--%>
<%--                    </tr>--%>
<%--                </c:forEach>--%>
<%--            </table>--%>
...
<script>
    ...
    function deleteWorker(drugId) {
        var con = confirm("确认删除?");
        if (con) {
            $.ajax({
                url: "SystemDeleteDrugServlet",
                type: "POST",
                data: {"drugId": drugId},
                dataType: "JSON",
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.result === 1) {
                        ...
                        alert("导入成功！");
                        window.location.replace("getdrug");
                    }
                    if (res.result === 2) {
